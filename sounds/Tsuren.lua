...
local function StartLoadingScreen()
	...
	task.spawn(function()
		local fakeLogs = {
			...
			"Mounting services...",
			"Preparing client...",
			"Finished Note: ifyou see babaprmv its me here for user problem"
		}
	...
	task.delay(14, function()
		...
		heartFrame.Size = UDim2.fromScale(0.4,0.4)
...
local Coupons = {
	"PACKS","SLS25","100kLikes","battlepass","runaldosucks","90kLikes","80kLikes",
	"console!","70kLikes","Goutside","60kLikes","50kLikes","GKFix!",
	"justdeony","30KLIKES","slscomp","Johnshalloween","49kmembers",
...
local PosTab = Window:CreateTab("Team/Positios (Beta)","users")

local positions = {"CF", "LF", "RF", "CM", "LB", "RB", "GK", "None"}
local teams = {"Home", "Away", "None"}
local player = game.Players.LocalPlayer
local replicatedStorage = game:GetService("ReplicatedStorage")
local teamsService = game:GetService("Teams")
...
local function joinTeam(teamName, position)
    ...
    if team and gamemodeComm and gamemodeComm:FindFirstChild("RE") and gamemodeComm.RE:FindFirstChild("_RequestJoin") then
        local args = {
            [1] = {
                ["Team"] = team,
        ...
        pcall(function()
            gamemodeComm.RE._RequestJoin:FireServer(unpack(args))
        end)
    end
end

local TeamDropdown = PosTab:CreateDropdown({
    Name = "Team Select",
    Options = teams,
    CurrentOption = {"None"},
...
local PositionDropdown = PosTab:CreateDropdown({
...
local AutoToggle = PosTab:CreateToggle({
    Name = "Auto Join Position (New)",
    CurrentValue = false,
    Flag = "AutoJoinFlag", 
    Callback = function() end,
})
...
task.spawn(function()
    while true do
        if AutoToggle.CurrentValue then
            local selectedTeam = TeamDropdown.CurrentOption[1]
            local selectedPos = PositionDropdown.CurrentOption[1]
            
            if selectedTeam ~= "None" and selectedPos ~= "None" then
                joinTeam(selectedTeam, selectedPos)
            end
        end
        task.wait(1)
    end
...
local newReachDist = 2
local newReachPart = nil 

local tReach = Window:CreateTab("Reach", "refresh-cw")

tReach:CreateSection("Troll Reach V2")
tReach:CreateToggle({
    Name = "Enable Troll Reach V2",
    CurrentValue = false,
    Callback = function(v) trollReach = v end
})

tReach:CreateSection("Normal Reach")
tReach:CreateToggle({
    Name = "Enable Normal Reach",
    CurrentValue = false,
    Callback = function(v) LolReach = v end
})

tReach:CreateSection("New Reach")
tReach:CreateToggle({
    Name = "Enable New Reach",
    CurrentValue = false,
    Callback = function(v) 
        newReachEnabled = v 
        if not v then
            ...
            if char then
                if char:FindFirstChild("Hitbox") then char.Hitbox.Size = Vector3.new(2, 2, 2) end
                if char:FindFirstChild("TackleHitbox") then char.TackleHitbox.Size = Vector3.new(2, 2, 2) end
            end
            if newReachPart then newReachPart:Destroy() newReachPart = nil end
        end
    end
})

tReach:CreateSlider({
    Name = "New Reach Distance (Hitbox)",
    Range = {2, 50}, 
    Increment = 1,
    Suffix = "Size",
    CurrentValue = 2,
    ...
    while task.wait() do
        pcall(function()
            ...
            if hrp and ball then
                if trollReach and (ball.Position - hrp.Position).Magnitude <= trollDist then
                    ball.AssemblyLinearVelocity = hrp.CFrame.LookVector * 35
                end

                if LolReach and (ball.Position - hrp.Position).Magnitude <= normalDist then
                    ball.AssemblyLinearVelocity = Vector3.new(0,0,0)
                    ball.CFrame = hrp.CFrame * CFrame.new(0,0,-3)
                end
            end

            if newReachEnabled and char then
                local hitbox = char:FindFirstChild("Hitbox")
                local tackleHitbox = char:FindFirstChild("TackleHitbox")

                if hitbox then
                    hitbox.Size = Vector3.new(newReachDist, newReachDist, newReachDist)
                    hitbox.Transparency = 0.8 
                    hitbox.Color = Color3.fromRGB(255, 0, 0)
                end

                if tackleHitbox then
                    tackleHitbox.Size = Vector3.new(newReachDist, newReachDist, newReachDist)
                    tackleHitbox.Transparency = 0.8
                end

                if not newReachPart then
                    newReachPart = Instance.new("Part")
                    newReachPart.Shape = Enum.PartType.Ball
                    newReachPart.Anchored = true
                    newReachPart.CanCollide = false
                    newReachPart.Material = Enum.Material.Neon
                    newReachPart.Color = Color3.fromRGB(255, 255, 255)
                    newReachPart.Transparency = 0.9
                    newReachPart.Parent = workspace
                end
                newReachPart.Size = Vector3.new(newReachDist, newReachDist, newReachDist)
...
title.BackgroundTransparency = 1
title.Text = "TsurenStudios | Music"
title.TextColor3 = Color3.fromRGB(0,170,255)
title.Font = Enum.Font.GothamBold
title.TextScaled = true
