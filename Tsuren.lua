...
local positions = {"CF","GK","CM","LB","RB","RW","LW"}

local tTeamPos = Window:CreateTab("Team & Position", "users")

-- UI LABELS
local teamLabel = tTeamPos:CreateParagraph({
    Title = "Team",
    Content = LocalPlayer.Team and LocalPlayer.Team.Name or "None"
})

local positionLabel = tTeamPos:CreateParagraph({
    Title = "Position",
    Content = LocalPlayer:GetAttribute("TeamPosition") or "None"
})
...
-- REMOTE (MENU BYPASS – ASIL ÖNEMLİ KISIM)
local JoinRemote
pcall(function()
    JoinRemote =
        ReplicatedStorage:WaitForChild("__GamemodeComm")
        :WaitForChild("RE")
        :WaitForChild("_RequestJoin")
end)

-- STATE
local selectedTeam = "Home"
local selectedPosition = positions[1]
...
local function joinTeam(teamName)
    ...
    -- Asıl çalışan yol (MENÜ BYPASS)
    if JoinRemote then
        pcall(function()
            JoinRemote:FireServer(teamName)
        end)
    end
...
task.spawn(function()
    while task.wait(1) do
        if autoJoin then
            -- TEAM FIX
            if not LocalPlayer.Team or LocalPlayer.Team.Name ~= selectedTeam then
                joinTeam(selectedTeam)
            end

            -- POSITION FIX
            if LocalPlayer:GetAttribute("TeamPosition") ~= selectedPosition then
...
tTeamPos:CreateSection("Manual Team")

tTeamPos:CreateButton({
    Name = "Join Home Team",
    Callback = function()
        joinTeam("Home")
    end
...
tTeamPos:CreateDivider()

tTeamPos:CreateDropdown({
...
tTeamPos:CreateToggle({
    Name = "Auto Join Team & Position",
    CurrentValue = false,
    Callback = function(v)
        autoJoin = v
    end
